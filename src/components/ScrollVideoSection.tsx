...
export default function ScrollVideoSection() {
  ...
  // States for device detection and loading
  const [assetPath, setAssetPath] = useState<string | null>(null);
  const [bufferReady, setBufferReady] = useState(false);
  const [globalProgress, setGlobalProgress] = useState(0);
  ...
  // 1. DEVICE DETECTION & DYNAMIC PRELOAD (Replaces the static top-level preload)
  useEffect(() => {
    // Detect if we are on a mobile screen
    const isMobile = window.innerWidth < 768;
    
    // Choose the folder based on device
    const path = isMobile 
      ? "/assets/mobile-frames/frames_" 
      : "/assets/scroll-frames/frames_";
    ...
    // Dynamically preload the first 50 frames into the browser cache
    for (let i = 0; i < BUFFER_CRITICAL_COUNT; i++) {
      preload(`${path}${getFrameString(i)}${EXTENSION}`, { as: "image" });
    }
  }, []);
  ...
  const drawFrame = (index: number) => {
    ...
    // Because Option 2 uses native portrait frames for mobile and landscape for desktop,
    // we can safely use Math.max (cover) for all devices without cutting off the subject.
    const scale = Math.max(canvasWidth / img.width, canvasHeight / img.height);
  ...
  // 3. ULTRA-FAST BACKGROUND STREAMING PIPELINE
  useEffect(() => {
    // Do not start loading until the device path is determined
    if (!assetPath) return; 
  ...
  // 4. ORIGINAL GSAP TIMELINE LOGIC RESTORED
  useGSAP(() => {
    if (!bufferReady || !containerRef.current || !canvasWrapperRef.current) return;

    const scrollerNode = scrollerContext?.current || window;

    // Restored your exact GSAP tween with snap: "frame" and ease: "none"
    const tl = gsap.to(frameRef.current, {
      frame: FRAME_COUNT - 1,
      snap: "frame",
      ease: "none", 
      onUpdate: () => drawFrame(frameRef.current.frame),
    });

    // Restored your exact ScrollTrigger with scrub: 1.2
    ScrollTrigger.create({
      trigger: containerRef.current,
      scroller: scrollerNode, 
      start: "top top",
  ...
  return (
    // Restored your exact height (2000vh) to ensure parent stacking contexts don't break
    <section 
      ref={containerRef} 
      className="relative w-full bg-black h-[2000vh]"
    >
      <div 
        ref={canvasWrapperRef} 
        className="sticky top-0 w-full h-screen overflow-hidden flex items-center justify-center bg-black"
      >
        ...
        {/* Restored your original canvas classes */}
        <canvas 
          ref={canvasRef} 
